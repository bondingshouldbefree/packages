...
static int fh_parse_xml_line(const char *xml_line, struct fh_cmd *fh_cmd) {
    ...
    if (strstr(xml_line, "vendor=\"meig\"")) {
...
static int fh_send_cmd(struct fh_data *fh_data, const struct fh_cmd *fh_cmd) {  
    ...
    if (strstr(fh_cmd->cmd.type, "vendor")) {
        ...
        snprintf(xml_buf + strlen(xml_buf), xml_size, 
            ...
            fh_cmd->erase.PAGES_PER_BLOCK, fh_cmd->erase.SECTOR_SIZE_IN_BYTES,fh_cmd->erase.start_sector+fh_cmd->erase.num_partition_sectors-1,
            fh_cmd->erase.num_partition_sectors, fh_cmd->erase.physical_partition_number, fh_cmd->erase.start_sector);
        else
        snprintf(xml_buf + strlen(xml_buf), xml_size, 
...
static int fh_send_rawmode_image(struct fh_data *fh_data, const struct fh_cmd *fh_cmd, unsigned timeout) {
   ...
   // dbg_time("send %s, filesize=%zd\n", unix_filename, filesize);
    fprintf(stdout,"send %s, filesize = %zd\n",unix_filename,filesize);
    int idx = -1;
...
static int fh_process_sbl(struct fh_data *fh_data, int opt)
{
    ...
    struct fh_cmd fh_rx_cmd;
    unsigned int x;
    dbg_time("cmd count is:%d",fh_data->fh_cmd_count);
    for (x = 0; x < fh_data->fh_cmd_count; x++) {
        ...
        if (strstr(fh_cmd->cmd.type, "program") &&
            ...
            opt == SBL_OPT_PROGRAM && strcmp(fh_cmd->program.filename,"sbl1.mbn") == 0){
            dbg_time("cmd count is:%d",x);
            goto do_program;
            }
    }
...
do_erase:
    ...
    return 0;
do_program:
    dbg_time("before file name is:%s",fh_cmd->program.filename);
//    strcpy(fh_cmd->program.filename,"sbl1.mbn");
  //  dbg_time("after file name is:%s",fh_cmd->program.filename);
    fh_send_cmd(fh_data, fh_cmd);
...
int firehose_main (const char *firehose_dir, void *usb_handle, unsigned qusb_zlp_mode) {
    ...
    char *erase_partitions_file = NULL; //zqy qdd 
    struct fh_cmd fh_rx_cmd;
    ...
    #if 0
    /*zqy add erase operation for slm750R2 20210419 start*/
    if(qfile_find_xmlfile(firehose_dir,"erase_partitions",&erase_partitions_file) != 0){
       dbg_time("retrieve erase_partitions file failed\r\n");
       error_return();
    }
    snprintf(rawprogram_full_path,sizeof(rawprogram_full_path),"%s/%s",firehose_dir,erase_partitions_file);
    free(erase_partitions_file);
    
    if(access(rawprogram_full_path,R_OK)){
        dbg_time("fail to access %s, erron: %d (%s) \n",rawprogram_full_path,errno,strerror(errno));
        error_return();
    }
    fh_parse_xml_file(fh_data,rawprogram_full_path);
    memset(&rawprogram_full_path,0,sizeof(rawprogram_full_path));
    //fprintf(stdout," erase fh_cmd_count is:%d",fh_data->fh_cmd_count);
    dbg_time("erase cmd count is:%d",fh_data->fh_cmd_count);
    #endif
    /*zqy add erase operation for slm750R2 20210419 end*/	
    ...
    fh_parse_xml_file(fh_data, rawprogram_full_path);
   // fprintf(stdout,"erase and program fh_cmd_count is:%d",fh_data->fh_cmd_count);
    if (fh_data->fh_cmd_count == 0)
        error_return();
   //dbg_time(stdout,"fh_cmd_count is:%d",fh_data->fh_cmd_count);
    dbg_time("erase+program cmd count is:%d",fh_data->fh_cmd_count);
    for (x = 0; x < fh_data->fh_cmd_count; x++) {
