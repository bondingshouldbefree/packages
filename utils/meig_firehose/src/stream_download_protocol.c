...
/*
set download flag in module, meig custom command, 
if flag : reboot, module will enter DM
if not flag: reboot normal
*/
static int handle_meig_download_flag(unsigned char mode) {
	//byte mode = 1;
...
int stream_download(const char *firehose_dir, void *usb_handle, unsigned qusb_zlp_mode)
{
    ...
    /*
    hello packet will set dload flag in module, when upgrade interrup, restart module,module will enter dm(meig sbl)
    */
    ...
    if (handle_parti_tbl(0) == false) {
        ...
        */
        if(handle_meig_download_flag(1) == false) {
            dbg_time("Set Meig download flag failed\n");
        }
        else {
            dbg_time("Set Meig download flag successfully\n");
        }
    }
