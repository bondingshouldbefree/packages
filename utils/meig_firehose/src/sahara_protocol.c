...
#define dbg( log_level, fmt, arg... )   do {dbg_time(fmt "\n", ## arg);} while (0) 
    ...
    do                                                                      \
    {                                                                       \
         fprintf(stdout,"%s: " fmt, firehos_get_time(), ##arg);            \
         fflush(stdout);                                                    \
         if(loghandler)                                                     \
             fprintf(loghandler,"%s: " fmt,firehose_get_time(), ##arg);    \
    }while(0)                                                              
#endif
//do {dbg_time(fmt "\n", ## arg);} while (0)
...
int sahara_main(const char *firehose_dir, void *usb_handle, int edl_mode_05c69008) {
    ...
    if(qfile_find_xmlfile(firehose_dir, "prog_nand_firehos", &prog_nand_firehose_filename) != 0
